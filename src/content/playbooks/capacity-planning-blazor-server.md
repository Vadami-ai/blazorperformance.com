...
description: "How to size app nodes, Azure SignalR units, and message budget for a Blazor Server workload, with every formula shown, from measurement to validation."
date: 2026-08-14
readingTime: "12 min"
---
...
inputs come from, and that's the rest of this playbook.
...
- Multiply by **tabs per user**. Enterprise users keep the app open in more than one
  tab more often than you'd expect; 1.2 to 1.5 is a typical measured range. Each tab
  is a full circuit, not a free rider.
...
network churn or rolling restarts, 5 to 15% extra circuits in memory is realistic.
This is governed by `DisconnectedCircuitMaxRetained` and the retention period, so
know your configured values.
...
Enterprise Blazor Server apps commonly land in the **0.3 to 1.5 MB per circuit**
range. If you measure above ~2 MB, fix state management before buying hardware.
That's almost always cheaper.
...
**25 to 50% is a sane starting band**, tuned to your measured baseline from step 2's
idle snapshot.
...
20 to 40% spare is typical. At small node counts, use the N+1 sanity check: with 3
...
2. **Messages**: every UI interaction round-trips messages, and units include a
   ...
   typically generate **2 to 5 messages per user-minute**; chatty real-time
   dashboards far more.
...
## Step 7: Validate, because the plan is a hypothesis
